...
bool MIDISequencer::GetNextEventTimeMs ( double *t )
{
    ...
    if ( f )
    {
		...
		int clicks = state.multitrack->GetClksPerBeat();
		if (clicks == 0) 
		{
			//printf("\nMIDISequencer::GetNextEventTimeMs clicks NULL \n");
			
		}else 
		{
			//printf("\nMIDISequencer::GetNextEventTimeMs clicks %d \n", clicks);
		}
...
bool MIDISequencer::GetNextEvent ( int *tracknum, MIDITimedBigMessage *msg )
{
    ...
    if ( state.iterator.GetCurEventTime ( &t ) )
    {
		...
		//printf("\n MIDISequencer::GetNextEvent new_time_ms %f\n", new_time_ms);
